...
<head>
    ...
    <title>NFE on WEB :: Orçamentos</title>
                ...
                <div class="box-body table-responsive no-padding">
                    <table class="table table-hover">
                       ...
                       <?php
                            ...
                            $buscaCliente = mysqli_query($conecta, "SELECT A.*, B.obs FROM clientes A, orcamentos B WHERE B.fkIdCliente = A.pkId AND B.pkId = ".base64_decode($_GET['ref']));
                       ...
                       <tr>
                            <td colspan="6">
                                <h3 style="text-align:center;margin-top: 0px;">ORÇAMENTO</h3>
                        ...
                        <?php
                            $query=mysqli_query($conecta, "SELECT A.*, B.nome AS NomeProduto FROM orcamentos_itens A, produtos B WHERE B.pkId = A.fkIdProduto AND A.fkIdOrcamento=".base64_decode($_GET['ref'])) or die(mysql_error());
                        ...
                        <tr>
                            ...
                            <td style="text-align:right; padding-right:25px; font-size: 12px"><?php echo number_format($result['precoVenda'], 2, ',', '.') ?></td>
                            <td style="text-align:right; padding-right:25px; font-size: 12px"><?php echo number_format($result['quantidade'], 2, ',', '.') ?></td>
                            <td style="text-align:right; padding-right:25px; font-weight:bolder; font-size: 12px"><?php echo number_format($result['precoVenda']*$result['quantidade'], 2, ',', '.') ?></td>
                        </tr>
                        <?php } ?>
                    </table>
                    ...
                    <table class="table table-hover">
                        <tr>
                            <?php
                                $query=mysqli_query($conecta, "SELECT * FROM orcamentos WHERE pkId=".base64_decode($_GET['ref'])) or die(mysql_error());
                                $result=mysqli_fetch_assoc($query);
                            ?>
                            <td>
                                <h4 style="text-align:center">Totais:</h4>
                                <br>
